# Brand asset sources

These SVGs are cached from the [SVGL registry](https://svgl.app/) so the marketing site does not depend on third-party requests at runtime.

- `github.svg`: `https://svgl.app/library/github_light.svg`
- `gitlab.svg`: `https://svgl.app/library/gitlab.svg`
- `linear.svg`: `https://svgl.app/library/linear.svg`
- `atlassian.svg`: `https://svgl.app/library/atlassian.svg` (used for Jira, an Atlassian product)
- `azure.svg`: `https://svgl.app/library/azure.svg` (used for Azure DevOps)

The respective companies retain all trademark rights.
