...
export default function GettingStartedPage() {
  return <DocsShell breadcrumb="Getting started" title="Build your first integration" description="Go from package installation to an inspectable, idempotency-coordinated provider write. This guide uses Linear, but the client lifecycle is identical for every adapter." toc={[{ id: "requirements", label: "Requirements" }, { id: "install", label: "Install" }, { id: "configure", label: "Configure" }, { id: "read", label: "Read" }, { id: "write", label: "Write safely" }, { id: "production", label: "Production checklist" }]}>
    ...
    <section id="production"><h2>Production checklist</h2><ul className="docs-checklist"><li>Use a durable <code>IdempotencyStore</code> with an atomic claim across processes.</li><li>Inspect every warning before setting <code>acceptWarnings: true</code>.</li><li>On <code>WorkConflictError</code>, discard the plan and prepare again.</li><li>On <code>WorkAmbiguousCommitError</code>, reconcile provider state before another write.</li><li>Pass <code>AbortSignal</code> from request or job cancellation.</li><li>Check <code>work.capabilities</code> before exposing optional actions.</li></ul></section>
