...
export const metadata: Metadata = { title: "Providers", description: "Configure and compare Work SDK provider adapters.", alternates: { canonical: "/docs/providers" } };
...
export default function ProvidersPage() {
  return <DocsShell breadcrumb="Providers" title="One contract, honest differences" description="Adapters normalize the portable work-item model while preserving provider-native state names, raw payloads, capabilities, and failure behavior. The goal is portability without pretending every tracker works the same way." toc={[{ id: "imports", label: "Imports" }, { id: "matrix", label: "Capability matrix" }, { id: "github", label: "GitHub" }, { id: "linear", label: "Linear" }, { id: "jira", label: "Jira" }, { id: "azure", label: "Azure DevOps" }, { id: "escape-hatches", label: "Escape hatches" }]}>
    ...
    <section id="matrix"><h2>Capability matrix</h2><div className="docs-table-wrap"><table><thead><tr><th>Capability</th><th>GitHub</th><th>Linear</th><th>Jira</th><th>Azure DevOps</th></tr></thead><tbody>
      <tr><td>Create / update</td><td>Yes</td><td>Yes</td><td>Yes</td><td>Yes</td></tr><tr><td>Comments</td><td>Markdown</td><td>Markdown</td><td>ADF</td><td>Markdown</td></tr><tr><td>Custom states</td><td>No</td><td>Yes</td><td>Yes</td><td>Yes</td></tr><tr><td>Priorities</td><td>No universal field</td><td>Native</td><td>Native</td><td>1–4 mapping</td></tr><tr><td>Multiple assignees</td><td>Yes</td><td>No</td><td>No</td><td>No</td></tr><tr><td>Parent links</td><td>No</td><td>Yes</td><td>Yes</td><td>Yes</td></tr><tr><td>Search</td><td>Not in adapter</td><td>GraphQL filters</td><td>JQL</td><td>WIQL</td></tr><tr><td>Concurrency</td><td>Readback revision</td><td>Readback revision</td><td>Readback revision</td><td>Native <code>/rev</code> test</td></tr>
    </tbody></table></div></section>
    <section id="github"><h2>GitHub Issues</h2><CodeBlock code={`github({\n  owner: "acme",\n  repo: "web",\n  token: process.env.GITHUB_TOKEN,\n})`} /><p>The adapter excludes pull requests from issue results, maps open/closed reasons, verifies fields through readback, and supports multiple assignees. GitHub Projects fields are intentionally outside the portable issue contract.</p></section>
    <section id="linear"><h2>Linear</h2><CodeBlock code={`linear({\n  apiKey: process.env.LINEAR_API_KEY!,\n  teamId: "team-id",\n})`} /><p>Linear uses cursor pagination, one assignee, native priorities, and team-specific workflow states. The adapter resolves state and label names against provider metadata before a mutation.</p></section>
    <section id="jira"><h2>Jira Cloud</h2><CodeBlock code={`jira({\n  baseUrl: "https://acme.atlassian.net",\n  email: process.env.JIRA_EMAIL!,\n  apiToken: process.env.JIRA_API_TOKEN!,\n  projectKey: "ENG",\n})`} /><p>Jira status changes are transitions, not ordinary field updates. The adapter discovers available transitions and converts conservative text content to Atlassian Document Format.</p></section>
