...
export default function SafeWritesPage() {
  return <DocsShell breadcrumb="Concepts / Safe writes" title="Prepare. Inspect. Commit." description="Work SDK turns a provider mutation into a reviewable value before it becomes a side effect. Integrity, optimistic concurrency, and idempotency each protect a different failure boundary." toc={[{ id: "why", label: "Why a protocol" }, { id: "anatomy", label: "Plan anatomy" }, { id: "integrity", label: "Integrity" }, { id: "concurrency", label: "Concurrency" }, { id: "idempotency", label: "Idempotency" }, { id: "warnings", label: "Warnings" }]}>
    ...
    <section id="idempotency"><h2>Idempotency</h2><p>The default memory store coordinates retries inside one process. Serverless, clustered, and job systems must supply a durable store. Keys should identify the business event, for example <code>github:webhook:delivery-123:comment</code>, not the attempt. Work SDK binds each key to the normalized provider, action, target, and input.</p><div className="docs-rule"><strong>Same key + same intent</strong><span>Return the stored receipt with <code>replayed: true</code>.</span></div><div className="docs-rule"><strong>Same key + different intent</strong><span>Fail with <code>WorkConflictError</code> before a provider mutation.</span></div><DocsCallout tone="warning"><p>Legacy stored receipts without an intent binding fail closed after upgrading. Use a new business key or reconcile and migrate the old receipt deliberately.</p></DocsCallout></section>
