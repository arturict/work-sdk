...
export default function ErrorsPage() {
  return <DocsShell breadcrumb="Reference / Errors" title="Normalized errors" description="Every provider maps transport and domain failures into stable classes. Branch on classes or codes, retain the provider context, and choose retry behavior deliberately." toc={[{ id: "hierarchy", label: "Hierarchy" }, { id: "fields", label: "Error fields" }, { id: "handling", label: "Handling" }, { id: "retry", label: "Retry policy" }, { id: "redaction", label: "Redaction" }]}>
    ...
    <section id="retry"><h2>Retry policy</h2><ul><li><strong>Rate limit:</strong> retry after <code>retryAfterMs</code>, add jitter, preserve the idempotency key.</li><li><strong>Network:</strong> retry only with a stable idempotency key or reconciliation strategy.</li><li><strong>Conflict:</strong> never retry the same plan; prepare and approve again.</li><li><strong>Validation/unsupported:</strong> do not retry unchanged input.</li><li><strong>Authentication:</strong> refresh credentials before another attempt.</li></ul><DocsCallout><p>A timeout cannot prove whether the remote provider committed a write. Idempotency protects SDK-level replays when the first result was recorded; reconciliation is still needed for the narrow “provider succeeded, receipt persistence failed” window.</p></DocsCallout></section>
